...
"""
Unit tests for the TagService layer.
"""
...
class TestTagService:
    """Test cases for TagService layer."""
    ...
    def service_with_mock_repo(self, mock_session_async, mock_repository):
        ...
        service = TagService(mock_session_async)
    ...
    @pytest.mark.asyncio
    async def test_create_tag_success(self, service_with_mock_repo, mock_repository, sample_data, existing_tag):
        """Test successful tag creation through service."""
        # Arrange
        new_item = TagCreate(**sample_data)
        mock_repository.create.return_value = existing_tag
    ...
    @pytest.mark.asyncio
    async def test_get_tag_by_id_success(self, service_with_mock_repo, mock_repository, existing_tag):
        """Test successful retrieval of tag by ID."""
        # Arrange
        mock_repository.get_by_id.return_value = existing_tag
    ...
    @pytest.mark.asyncio
    async def test_get_tag_by_id_not_found(self, service_with_mock_repo, mock_repository):
    ...
    @pytest.mark.asyncio
    async def test_get_all_success(self, service_with_mock_repo, mock_repository, multiple_tag, pagination_request):
        """Test successful retrieval of all tag."""
        # Arrange
        mock_repository.get_all.return_value = (multiple_tag, len(multiple_tag))
        ...
        assert len(result.items) == len(multiple_tag)
        assert result.total == len(multiple_tag)
        mock_repository.get_all.assert_called_once_with(pagination_request)

    

    @pytest.mark.asyncio
    async def test_search_success(self, service_with_mock_repo, mock_repository, multiple_tag, pagination_request):
        """Test successful search with filters."""
        # Arrange
        filters = TagFilter()
        mock_repository.search.return_value = (multiple_tag, len(multiple_tag))
    ...
    @pytest.mark.asyncio
    async def test_update_tag_by_id_success(self, service_with_mock_repo, mock_repository, updated_tag_model):
        """Test successful tag update."""
        # Arrange
        mock_repository.update_by_id.return_value = updated_tag_model
        
        # Act
        result = await service_with_mock_repo.update_by_id(1, TagUpdate())
        
        # Assert
        assert isinstance(result, TagResponse)
        mock_repository.update_by_id.assert_called_once_with(1, TagUpdate().model_dump(exclude_unset=True))
