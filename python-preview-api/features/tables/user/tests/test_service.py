...
"""
Unit tests for the UserService layer.
"""
...
class TestUserService:
    """Test cases for UserService layer."""
    ...
    def service_with_mock_repo(self, mock_session_async, mock_repository):
        ...
        service = UserService(mock_session_async)
    ...
    @pytest.mark.asyncio
    async def test_create_user_success(self, service_with_mock_repo, mock_repository, sample_data, existing_user):
        """Test successful user creation through service."""
        # Arrange
        new_item = UserCreate(**sample_data)
        mock_repository.create.return_value = existing_user
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_id_success(self, service_with_mock_repo, mock_repository, existing_user):
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_id_not_found(self, service_with_mock_repo, mock_repository):
    ...
    @pytest.mark.asyncio
    async def test_get_all_success(self, service_with_mock_repo, mock_repository, multiple_user, pagination_request):
        """Test successful retrieval of all user."""
        # Arrange
        mock_repository.get_all.return_value = (multiple_user, len(multiple_user))
        ...
        assert len(result.items) == len(multiple_user)
        assert result.total == len(multiple_user)
        mock_repository.get_all.assert_called_once_with(pagination_request)

    

    @pytest.mark.asyncio
    async def test_search_success(self, service_with_mock_repo, mock_repository, multiple_user, pagination_request):
        """Test successful search with filters."""
        # Arrange
        filters = UserFilter()
        mock_repository.search.return_value = (multiple_user, len(multiple_user))
    ...
    @pytest.mark.asyncio
    async def test_update_user_by_id_success(self, service_with_mock_repo, mock_repository, updated_user_model):
        """Test successful user update."""
        # Arrange
        mock_repository.update_by_id.return_value = updated_user_model
        
        # Act
        result = await service_with_mock_repo.update_by_id(1, UserUpdate())
        
        # Assert
        assert isinstance(result, UserResponse)
        mock_repository.update_by_id.assert_called_once_with(1, UserUpdate().model_dump(exclude_unset=True))
