...
"""
Unit tests for the DocumentService layer.
"""
...
class TestDocumentService:
    """Test cases for DocumentService layer."""
    ...
    def service_with_mock_repo(self, mock_session_async, mock_repository):
        """Create DocumentService with mocked repository."""
        service = DocumentService(mock_session_async)
    ...
    @pytest.mark.asyncio
    async def test_create_document_success(self, service_with_mock_repo, mock_repository, sample_data, existing_document):
        """Test successful documents creation through service."""
        # Arrange
        new_item = DocumentCreate(**sample_data)
        mock_repository.create.return_value = existing_document
    ...
    @pytest.mark.asyncio
    async def test_get_document_by_id_success(self, service_with_mock_repo, mock_repository, existing_document):
        """Test successful retrieval of document by ID."""
        # Arrange
        mock_repository.get_by_id.return_value = existing_document
    ...
    @pytest.mark.asyncio
    async def test_get_document_by_id_not_found(self, service_with_mock_repo, mock_repository):
    ...
    @pytest.mark.asyncio
    async def test_get_all_success(self, service_with_mock_repo, mock_repository, multiple_documents, pagination_request):
        """Test successful retrieval of all documents."""
        # Arrange
        mock_repository.get_all.return_value = (multiple_documents, len(multiple_documents))
        ...
        assert len(result.items) == len(multiple_documents)
        assert result.total == len(multiple_documents)
        mock_repository.get_all.assert_called_once_with(pagination_request)

    

    @pytest.mark.asyncio
    async def test_search_success(self, service_with_mock_repo, mock_repository, multiple_documents, pagination_request):
        """Test successful search with filters."""
        # Arrange
        filters = DocumentFilter()
        mock_repository.search.return_value = (multiple_documents, len(multiple_documents))
    ...
    @pytest.mark.asyncio
    async def test_update_document_by_id_success(self, service_with_mock_repo, mock_repository, updated_document_model):
        """Test successful document update."""
        # Arrange
        mock_repository.update_by_id.return_value = updated_document_model
        
        # Act
        result = await service_with_mock_repo.update_by_id(1, DocumentUpdate())
        
        # Assert
        assert isinstance(result, DocumentResponse)
        mock_repository.update_by_id.assert_called_once_with(1, DocumentUpdate().model_dump(exclude_unset=True))
