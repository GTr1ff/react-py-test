...
"""
Unit tests for the IngredientService layer.
"""
...
class TestIngredientService:
    """Test cases for IngredientService layer."""


    @pytest.fixture
    def mock_repository(self):
        """Create a mock IngredientRepository."""
        return AsyncMock(spec=IngredientRepository)

    @pytest.fixture
    def service_with_mock_repo(self, mock_session_async, mock_repository):
        """Create IngredientService with mocked repository."""
        service = IngredientService(mock_session_async)
    ...
    @pytest.mark.asyncio
    async def test_create_ingredient_success(self, service_with_mock_repo, mock_repository, sample_data, existing_ingredient):
        """Test successful ingredient creation through service."""
        # Arrange
        new_item = IngredientCreate(**sample_data)
        mock_repository.create.return_value = existing_ingredient
    ...
    @pytest.mark.asyncio
    async def test_get_ingredient_by_id_success(self, service_with_mock_repo, mock_repository, existing_ingredient):
        """Test successful retrieval of ingredient by ID."""
        # Arrange
        mock_repository.get_by_id.return_value = existing_ingredient
    ...
    @pytest.mark.asyncio
    async def test_get_ingredient_by_id_not_found(self, service_with_mock_repo, mock_repository):
    ...
    @pytest.mark.asyncio
    async def test_get_all_success(self, service_with_mock_repo, mock_repository, multiple_ingredient, pagination_request):
        """Test successful retrieval of all ingredient."""
        # Arrange
        mock_repository.get_all.return_value = (multiple_ingredient, len(multiple_ingredient))
        ...
        assert len(result.items) == len(multiple_ingredient)
        assert result.total == len(multiple_ingredient)
        mock_repository.get_all.assert_called_once_with(pagination_request)

    

    @pytest.mark.asyncio
    async def test_search_success(self, service_with_mock_repo, mock_repository, multiple_ingredient, pagination_request):
        """Test successful search with filters."""
        # Arrange
        filters = IngredientFilter()
        mock_repository.search.return_value = (multiple_ingredient, len(multiple_ingredient))
    ...
    @pytest.mark.asyncio
    async def test_update_ingredient_by_id_success(self, service_with_mock_repo, mock_repository, updated_ingredient_model):
        """Test successful ingredient update."""
        # Arrange
        mock_repository.update_by_id.return_value = updated_ingredient_model
        
        # Act
        result = await service_with_mock_repo.update_by_id(1, IngredientUpdate())
        
        # Assert
        assert isinstance(result, IngredientResponse)
        mock_repository.update_by_id.assert_called_once_with(1, IngredientUpdate().model_dump(exclude_unset=True))
